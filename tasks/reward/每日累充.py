...
class 每日累充(强化奖励任务基类):
    ...
    def _每日累充(self):
        if 是否为今天(self.上次确定任务结束时间): 
            调试器.info("强化奖励", "每日累充: 检查到今日充值已满额度")
            return
        if not self._检查是否要充值(): return
        调试器.info("强化奖励", "每日累充: 启动")
        ...
        if self.页面.创建_通用点击区域验证文字切换(
            ...
            需要充值金额=每日累充金额-金额
            if 需要充值金额<=0:
                调试器.info("强化奖励", "每日累充: 今日已充值金额 >= 每日累充金额")
    ...
    def _读取本周已充值天数(self) ->Optional[bool]:
        ...
        if not 领取结果:
            调试器.info("强化奖励", "未找到'领取'按钮")
            return
        
        count = len(领取结果)
        调试器.info("强化奖励", f"第一页领取数量: {count}")
        
        if count < 4:
            self.本周已充值天数 = count
            self.上次读取每周充值天数时间 = time.time()
            调试器.info("强化奖励", f"本周已充值天数: {self.本周已充值天数}")            
            return True
        

        if count >= 4:
            调试器.info("强化奖励", "领取数量>=4，拖拽查看后面天数")
            ...
            if 是否7天:
                ...
                if 领取结果文字 is None:
                    调试器.info("强化奖励", "拖拽后未识别到领取区域文字")
                    return
                
                领取结果 = 领取结果文字.find("领取", return_all=True)
                if not 领取结果:
                    调试器.info("强化奖励", "拖拽后未找到'领取'按钮")
                    return
                
                count2 = len(领取结果)
                self.本周已充值天数 = count2 + 3
                self.上次读取每周充值天数时间 = time.time()
                调试器.info("强化奖励", f"拖拽后领取数量: {count2}, 本周已充值天数: {self.本周已充值天数}")
                return True
    def _重置本周已充值天数(self):
        """
        ...
        调试器.info("强化奖励", f"本周六: {本周六}, 今天: {today}, 应充值天数: {应充值天数}, 实际充值天数: {self.本周已充值天数}")
        
        if self.本周已充值天数 > 应充值天数:
            调试器.info("强化奖励", f"充值天数异常({self.本周已充值天数} > {应充值天数})，重置为0")
            self.本周已充值天数 = 0
    def _领取连充奖励(self) ->Optional[bool]:
        调试器.info("强化奖励", "领取连充奖励")
        time.sleep(self.配置.默认等待秒)
        self.页面.线程.刷新截图()
    ...
    def 获取红包数据(self, 背包区域: Tuple[int, int, int, int], 
        ...
        for 配置 in 红包配置:
            调试器.info("红包图标", f"金额: {配置['金额']}")
            调试器.info("红包图标",  配置["图片"])
            图标 = self.辅助识别器.查找图片单结果(背包区域, 配置["图片"])
            if 图标 is None: 
                调试器.info("红包图标", f"红包查找失败")
            if 图标:
                调试器.info("红包图标", f"格子序号: {图标}")
                格子, 序号 = self.通用操作.查找目标所在格子(图标, 格子坐标集合)
                if 格子 is not None:
                    数量区域 = (格子[0] + 28, 格子[1] + 40, 格子[0] + 60, 格子[1] + 61)
                    数量 = self.辅助识别器.获取区域次数(数量区域, filter_config,放大倍数=3)
                    if 数量 is None:
                        数量=1
                    调试器.info("红包数量", f"格子序号: {序号}, 量: {数量}")
                    调试器.info("红包金额", 配置["金额"])
                    红包列表.append({
                        "金额": 配置["金额"],
                        "数量": 数量,
