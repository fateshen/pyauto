...
@任务定义(
    ...
    位置复查目标半径=11,
    工作时间开始=21,
    工作时间段列表="21:36-21:56,22:06-22:26",
    执行日期规则="日期3-23 & 星期1,3,5",
    提前进场秒数=0,
    目标旗子="金岩霸者旗",
    次数刷新区间列表 = "21:36-21:56,21:36-21:56,22:06-22:26",
    次数刷新区间条件 = "日期3-19 & 星期1,3,5|日期20-23 & 星期1,3,5|日期20-23 & 星期1,3,5" , # 第二个区间对应第二时段   
    状态_移动开始时间=0,
...
class 迷阵夺旗任务(战斗任务执行器):
    ...
    def 执行入口逻辑(self) -> bool:
        ...
        if self.任务状态.入口连续失败次数>=5:
            调试器.debug(self.调试分类, "连续失败5次，跳过执行")
            self.任务状态.剩余次数 = 0
            return False
        # 1. 检查剩余次数
        if self.任务状态.剩余次数 <= 0:
        ...
        调试器.debug(self.调试分类, f"今天是{今天.day}")
        if  今天.day>=19:
            调试器.debug(self.调试分类, "今天是19号以后，进入世界赛程")
            self.页面.创建_通用点击区域验证文字切换(
            self.游戏配置.区域.至尊联赛.迷阵夺旗世界赛程按钮区域.元组,
