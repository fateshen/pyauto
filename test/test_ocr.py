...
def test_ocr():
    ...
    多匹配=模板匹配.match_all_bypicture(截图, "小地图玩家色块.bmp",region=自检区域)
    print(f"   多匹配：{多匹配}")
    for 匹配 in 多匹配:
        print(  f"匹配x：{匹配.rect}")
    ...
    for i in range(3):
            ...
            属性名 = f"决战盟重页面玩家准备情况标签{index}"
            print(属性名)
            区域对象 = getattr(区域.决战盟重, 属性名, None)
            # 区域对象 =区域.陨圣.获取刷新标签(index)
           
            if 区域对象 is None:               
                continue
            识别区域 = 区域对象.元组
            ocr_result = 识别器.recognize_text(截图,识别区域,放大倍数=1)
            print(ocr_result)
    # print(f"   识别结果: {text}")
