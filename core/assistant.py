...
class 战斗辅助识别器:
    ...
    def 古剑类副本归属判定(self):
        ...
        归属名字 = 归属信息["名字"]
        归属服务器 = 归属信息["服务器号"]
        调试器.info("战斗助手", f"归属判定: '{归属文字}' → 名字='{归属名字}' 服务器='{归属服务器}'")
    ...
    def 宝库类副本归属判定(self):
        ...
        # 2. 检查"抢归属"按钮
        抢归属文字 = self.获取区域文字(
            self.游戏配置.区域.圣兽宝库.宝库深处页面抢归属按钮标签.元组
        )
        if not 抢归属文字 or "抢" not in 抢归属文字:
            return 归属情况.未知
        ...
        归属文字 = self.线程.文字识别器.recognize_text(
            self.线程.截图,
            self.游戏配置.区域.圣兽宝库.宝库深处页面归属玩家区域标签.元组,
            filter_config
        )
        if not 归属文字:
            if "抢" in 抢归属文字:
    ...
    def 主宰类副本归属判定(self):
        ...
        玩家名字 = self.获取玩家角色名称()
        玩家公会名字=self.线程.游戏配置.玩家.玩家公会名称
        if not 玩家名字:
            return 归属情况.未知
    
        # 2. 检查"抢归属"按钮
        抢归属文字 = self.获取区域文字(
            self.游戏配置.区域.主宰.主宰抢归属按钮区域.元组
        )
        if not 抢归属文字 or "抢" not in 抢归属文字:
            return 归属情况.未知
        ...
        归属行会文字 = self.线程.文字识别器.recognize_text(
            self.线程.截图,
            self.游戏配置.区域.主宰.主宰归属行会检查区域.元组,
            filter_config
        )
        if 归属行会文字 and 玩家公会名字:
            if 匹配分组关键字(归属行会文字,玩家公会名字):
                return 归属情况.归属公会
              
        归属文字 = self.线程.文字识别器.recognize_text(
            self.线程.截图,
            self.游戏配置.区域.主宰.主宰归属玩家检查区域.元组,
            filter_config
        )
        归属信息 = 解析玩家全名(归属文字)
        归属名字 = 归属信息["名字"]
