...
class 通用操作集:
    ...
    def 点击区域所有红点偏移模式(self, 检查区域:Tuple[int,int,int,int],偏移:Tuple[int,int,int,int]=(-30,5,-10,20),单击次数:int=2,  检查图片: str="红点1.bmp", 名称: str = "") -> bool:
        """
        查找红点并按偏移点击
        ...
        返回:
            是否找到并点击
        """
        红点集合 = self.线程.辅助识别器.查找图片多结果(检查区域, 检查图片)
        if not 红点集合:
            return False
    ...
    def 关闭中间小区域可能存在的窗口(self):
        ...
        关闭按钮集合=self.线程.模板匹配器.match_all_bypicture(
            ...
            "关闭按钮.bmp",
            0.8,            
            self.游戏配置.区域.主界面.关闭按钮搜寻中区域标签.元组,           
        )
        if not 关闭按钮集合:
            调试器.verbose("通用操作", "未找到关闭按钮")  
            return
