    ...
    def 获取任务配置(self, 任务名称: str) :
        ...
        for 任务 in self.任务排序列表:
            if 任务.任务名称 == 任务名称:
                return 任务
        return None
    def 获取任务状态_按名称(self, 任务名称: str):
        """根据任务名称获取任务状态"""
        for 任务 in self.任务排序列表:
            if getattr(任务, '任务名称', '') == 任务名称:
                任务ID = 任务.任务ID
                return self.任务状态映射.get(任务ID)
        return None
    # ==================== 主循环 ====================
                ...
                if self.辅助识别器.查找图片单结果(self.游戏配置.区域.图鉴强化.炼妖炉完成提示区域标签.元组,"炼妖炉.bmp"):
                    ...
                    for 任务 in self.强化奖励管理器.任务实例列表:
                        if 任务.任务ID == "lianyaolu":
                            ...
                            break
                # 检查炼妖炉
                感叹号区域= self.辅助识别器.查找图片单结果(self.游戏配置.区域.主界面.公共提示区域标签.元组,"感叹号提示.bmp")
                if 感叹号区域:
                    self.通用操作.点击区域(感叹号区域)
                    调试器.info("界面提示复查", f"找到感叹号")
                    time.sleep(self.游戏配置.默认等待秒*2)
                    self.刷新截图()
                    if self.辅助识别器.区域包含文字(self.游戏配置.区域.炼器宝阁.炼器宝阁被抢夺后冒号进入提示区域.元组,"宝阁|抢夺"):                        
                        宝阁状态=self.获取任务状态_按名称("炼器宝阁")                              
                        if 宝阁状态 is not None:
                            调试器.info("界面提示复查", f"炼器宝阁被抢夺")
                            宝阁状态.下次刷新时间=time.time()
                        self.通用操作.点击区域(self.游戏配置.区域.炼器宝阁.炼器宝阁被抢夺后进入确定提示区域.元组)
                        time.sleep(self.游戏配置.默认等待秒)
                        self.刷新截图()
                        self.通用操作.关闭中间可能存在的窗口()
                        time.sleep(self.游戏配置.默认等待秒)                        
                    
                    if self.辅助识别器.区域包含文字(self.游戏配置.区域.主界面.中间页面名称区域标签.元组,"活|动"):  
                        领取红包文字=self.辅助识别器.获取区域文字坐标(self.游戏配置.区域.各种活动.抢红包界面红包出现范围.元组)
                        if 领取红包文字:
                            文字区域集合=领取红包文字.find("点击领取",return_all=True)
                            if 文字区域集合:
                                for 文字区域 in 文字区域集合:                                      
                                    if self.页面.创建_通用点击区域验证图片切换(
                                        文字区域,
                                        self.游戏配置.区域.各种活动.抢红包界面红包出现抢红包按钮区域.元组,
                                        "抢红包按钮.bmp"
                                        ).执行():
                                        调试器.info("界面提示复查", f"找到红包")
                                        if self.页面.创建_通用点击图片验证文字切换(
                                            self.游戏配置.区域.各种活动.抢红包界面红包出现抢红包按钮区域.元组,
                                            "抢红包按钮.bmp",
                                            self.游戏配置.区域.各种活动.抢红包界面抢红包返回按钮.元组,
                                            "返|回",
                                        ).执行():     
                                            self.通用操作.点击区域(self.游戏配置.区域.各种活动.抢红包界面抢红包返回按钮.元组)                                    
                                            调试器.info("界面提示复查", f"抢红包成功")
                        time.sleep(self.游戏配置.默认等待秒)
                        self.刷新截图()
                        self.通用操作.关闭中间可能存在的窗口()
                        time.sleep(self.游戏配置.默认等待秒)    
                    self.界面信息抽查时间 = time.time()+300  
                    continue
